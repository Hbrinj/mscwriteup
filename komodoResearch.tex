...
%
\itl{BR\_SET\_REG} is called when the user changes a register value in a specific register bank. KoMoDo sends the register value and address to the \itl{Jimulator}. Where the \itl{Jimulator} updates its internal representation\\\\
%
...
%
\itl{BR\_BP\_GET}s exact functionality is dubiousgiven that one of the values used when KoMoDo sends this command is not explicitly set or defined, anywhere. The initial understanding is that it is used to retrieve the index of one of the 32 allowed breakpoints, followed by the breakpoint status. KoMoDo will send this when determining if a toggled breakpoint has been previously set.\\\\
%
