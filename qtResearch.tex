...
%
Initially before carrying out this research the plan was to use \itl{QT} for the front-end. As both KoMoDo and the other frameworks were explored, it became apparent that \itl{GTK} is heavily ingrained within KoMoDo, as such using \itl{QT} would mean one of two things; create a shim which transformed \itl{Glib} data structures to something \itl{QT} would understand or alternatively, remake KoMoDo from the ground up using \itl{QT}. Due to time constraints the latter was deemed not feasible and the former would produce additional undesired complexities. Consequently, \itl{GTK} is the only choice to make in this instance. Utilising \itl{MinGW} and \itl{MSys} will allow components like the \itl{Jimulator} to function as if they are being executed in a Linux environment. The jimulator, being the heart of KoMoDo is its own complex program\footnote{It'll need its own dissertation to convert into something modern}.
